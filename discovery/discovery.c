...
static void discovery_mdns_cb(const char *peer_id, const char *multiaddr, void *user_data) {
	struct Libp2pDiscovery* discovery = (struct Libp2pDiscovery*)user_data;
	if (discovery == NULL || peer_id == NULL || multiaddr == NULL) {
		return;
	}
	if (discovery->peer_id != NULL && strcmp(discovery->peer_id, peer_id) == 0) {
		return;
	}
	libp2p_discovery_record_observed_address(discovery, multiaddr);
	if (discovery->config.relay_enabled) {
		libp2p_discovery_add_relay_hint(discovery, multiaddr);
	}
}
...
static void* discovery_worker(void* arg) {
	...
	while (discovery->running) {
		if (discovery->mdns != NULL) {
			mdns_service_send_announcement(discovery->mdns);
			mdns_service_poll(discovery->mdns);
		}
...
int libp2p_discovery_set_identity(struct Libp2pDiscovery* discovery, const char* peer_id, const char* multiaddr) {
	...
	free(discovery->peer_id);
	free(discovery->local_multiaddr);
	discovery->peer_id = discovery_strdup(peer_id);
	discovery->local_multiaddr = discovery_strdup(multiaddr);
	return (discovery->peer_id != NULL && discovery->local_multiaddr != NULL);
}
...
int libp2p_discovery_start(struct Libp2pDiscovery* discovery) {
	...
	if (discovery->config.mdns_enabled && discovery->peer_id != NULL && discovery->local_multiaddr != NULL) {
		if (discovery->mdns == NULL) {
			discovery->mdns = mdns_service_new(discovery->peer_id, discovery->local_multiaddr, discovery_mdns_cb, discovery);
		}
		if (discovery->mdns != NULL && mdns_service_start(discovery->mdns) != 0) {
			mdns_service_free(discovery->mdns);
			discovery->mdns = NULL;
			return 0;
		}
	}
